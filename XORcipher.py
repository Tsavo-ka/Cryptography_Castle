...
# Function to convert text to 7-bit binary representation
def convert_bin(text):
    # Save each character's binary as an item in a list
    split = [format(ord(l), 'b') for l in text]

    # If any binary string has a length of < 7, add 0's to the start
...
# Function to convert binary representation to ASCII text
...
# Function to check if two characters match
def match(c1, c2):
    return c1 == c2
...
# Main function to run the XOR cipher program
def main():
    ...
    4. The encryptor will then iterate over the binary of the text and generate a 1 or 0 according to this table:
